...
// ===================== MAIN MENU =====================
bot.hears([/Ma'lumot/i, /О компании/i, /Ma'lumot/i, /О компании/i], (ctx) => {
  const lang = userLang[ctx.chat.id] || "uz";
  ctx.reply(getText(lang, 'company_info'));
});

bot.hears([/Aloqa/i, /Контакты/i, /Aloqa/i, /Контакты/i], (ctx) => {
  const lang = userLang[ctx.chat.id] || "uz";
...
bot.on(['photo', 'video'], (ctx) => {
  ...
  state.data.mediaFiles.push({
    ...
    mediaType: ctx.message.photo ? 'photo' : 'video',
...
bot.hears([/Orqaga/i, /Назад/i], (ctx) => {
  ...
  if (menu === 'admin') return ctx.reply(getText(lang, 'main_menu'), getMainMenu(lang));
  setCurrentMenu(ctx.chat.id, 'admin');
...
// ===================== CATALOG & OTHERS (qisqartirildi) =====================
// ... (oldingi katalog, edit, delete qismlari saqlanadi)
